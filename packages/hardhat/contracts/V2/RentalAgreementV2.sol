...
interface IReputation {
    function updateReputations(uint256 _agreementId, address _owner, address _renter, bool _successful) external;
}
...
contract RentalAgreement is ReentrancyGuard, Ownable {
    ...
    function completeAgreement(uint256 _agreementId) external nonReentrant {
        ...
        require(msg.sender == agreement.owner || msg.sender == agreement.renter, "Not authorized");
        require(agreement.isActive, "Agreement not active");
        require(block.timestamp >= agreement.startTime + agreement.rentalPeriod, "Rental period not over");

        // Optionally, perform inspection before completion
        bool isItemInGoodCondition = IInspection(inspectionContract).inspectItem(_agreementId);
        require(isItemInGoodCondition, "Item failed inspection");
        ...
        IReputation(reputationContract).updateReputations(_agreementId, agreement.owner, agreement.renter, true);
    }
    ...
    function raiseDispute(uint256 _agreementId) external {
        ...
        IDisputeResolution(disputeResolutionContract).initiateDispute(_agreementId);
    }
    ...
    function extendRentalPeriod(uint256 _agreementId, uint256 _additionalPeriod) external nonReentrant {
        Agreement storage agreement = agreements[_agreementId];
        require(msg.sender == agreement.owner || msg.sender == agreement.renter, "Not authorized");
        require(agreement.isActive, "Agreement not active");
    ...
    function updateInspectionContract(address _inspectionContract) external onlyOwner {
        inspectionContract = _inspectionContract;
    }

    function updateReputationContract(address _reputationContract) external onlyOwner {
        reputationContract = _reputationContract;
    }

    function updateDisputeResolutionContract(address _disputeResolutionContract) external onlyOwner {
        disputeResolutionContract = _disputeResolutionContract;
    }
}
