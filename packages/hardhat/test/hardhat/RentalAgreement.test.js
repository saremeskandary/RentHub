...
describe("RentalAgreement", function () {
  ...
  beforeEach(async function () {
    ...
    token = await mockTokenContract.deploy(accessRestriction.address); //"MockToken", "MTK", 18, ethers.utils.parseEther("1000")
    ...
    rentalDAO = await rentalDAOContract.deploy(accessRestriction.address, 200);
    ...
    let Distributor_ROLE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("APPROVED_CONTRACT_ROLE"));
    ...
    token.connect(distributer).mint(renter, ethers.utils.parseEther("10000"));
  });
...
//   });
  ...
  describe("ArrivalAgreement", function () {
      ...
      it("should allow the renter to confirm their arrival and start the agreement", async function () {
        ...
        //   await token.transfer(renter.address, ethers.utils.parseEther("5000"));
          await token.connect(renter).approve(rentalAgreement.address, ethers.utils.parseEther("6000"));
